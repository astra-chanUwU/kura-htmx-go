...
- [ ] As your super admin, open Admin → Tags. Search and filter the bounded inventory, then preview a rename and confirm its affected-post count and unchanged category before applying it. Preview a same-category merge with a duplicate relationship and confirm it deduplicates the post tags; try a cross-category merge and a rename collision and confirm both are rejected without changes. Reopen Audit and confirm the successful tag event retains actor, source/target categories, affected count, and reason.
