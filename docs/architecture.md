...
There is no background service in v1. Upload work can extract metadata and create a thumbnail inside the request, using a private temporary file followed by an atomic rename. Startup reconciles any private permanent-delete staging left by an interrupted request. If measurements later show that processing blocks normal use, a database-backed job table is the smallest justified next step.
...
- `audit_events`: role changes, draft quarantines, manual quarantines, restores, and their actor/target/post references.
...
Normal browse, direct post view, media serving, navigation, pool membership, favorites, downloads, and ZIP exports all exclude quarantined posts. An active super admin can inspect a quarantined post through the review view and can restore it to its recorded prior status or permanently delete it. Demoting a moderator or admin to viewer revokes that user's sessions and quarantines its non-deleted drafts in the same transaction; published uploads remain published, and later re-promotion does not restore those drafts automatically. Quarantine records preserve the media for review and do not imply permanent deletion.

Error responses use Kura's styled HTML error page for ordinary browser requests and accessible fragments or safe JSON where the request requires them. Post details carry validated, allow-listed context for browse/search, pools, My uploads, and super-admin oversight; previous/next neighbors are recomputed for the current actor and Escape/arrow handling avoids editable controls. Authentication forms share account-aware username/autocomplete identity, editable passkey defaults, and labeled recovery output while preserving WebAuthn origin/RP checks, password policy, recovery rotation, and fresh-passkey requirements.
